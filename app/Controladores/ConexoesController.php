...
class ConexoesController
{
    ...
    public function salvar(array $data): array
    {
        ...
        if ($data['tipo_banco'] === 'oracle') {
            ...
            // Para SQLite, armazenar o caminho no campo nome_banco também
            $data['nome_banco'] = $data['sqlite_path'] ?? $data['nome_banco'] ?? '';
            $data['host'] = $data['host'] ?: null;
            $data['porta'] = $data['porta'] ?: null;
            $data['usuario'] = $data['usuario'] ?: null;
        } elseif ($data['tipo_banco'] === 'odbc') {
            $parametrosExtras['odbc_dsn'] = $data['odbc_dsn'] ?? '';
            $data['nome_banco'] = $data['odbc_dsn'] ?? $data['nome_banco'] ?? '';
        }
