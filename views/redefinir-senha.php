<?php
/**
 * DMC DataLoad - Redefinir Senha
 * Tela pública onde o usuário digita a chave hex e a nova senha
 */
$loginBg = '';
...
<head>
  ...
  <title>Redefinir Senha - <?= htmlspecialchars($appNome) ?></title>
  <?php if ($appFavicon): ?>
  <link rel="icon" href="<?= $baseUrl . htmlspecialchars($appFavicon) ?>" type="image/x-icon">
  <?php endif; ?>
  <style>
    :root {
      --login-primary: #667eea; --login-primary-dark: #5a67d8; --login-primary-light: #7c8ff7;
      --login-accent: #764ba2; --login-bg: #eef1f8; --login-card-bg: rgba(255,255,255,0.97);
      --login-text: #1a1a2e; --login-muted: #6c757d; --login-radius: 20px;
      --login-input-radius: 14px; --login-transition: 0.3s cubic-bezier(0.4, 0, 0.2, 1);
    }
    ...
    .mobile-brand-logo {
      ...
      font-size: 1.8rem; color: white; margin-bottom: 0.75rem; box-shadow: 0 6px 20px rgba(102,126,234,0.3);
    }
    .login-form-side {
      display: flex; flex-direction: column; align-items: center; justify-content: center;
    ...
    .btn-toggle-pw {
      background: transparent; border: none; padding: 0.75rem 1rem; color: #b0b8c5;
      cursor: pointer; transition: all var(--login-transition); display: flex; align-items: center;
    }
    .btn-toggle-pw:hover { color: var(--login-primary); }
    ...
    .code-input {
      letter-spacing: 0.5rem; font-size: 1.5rem; text-align: center;
    ...
    @media (prefers-reduced-motion: reduce) { .login-card, .login-branding-overlay, .login-stagger-1, .login-stagger-2, .login-stagger-3, .login-stagger-4, .login-image-side::before, .login-image-side::after { animation: none !important; } .login-card::before { animation: none !important; } }
...
<body class="login-wrapper">
        ...
        <div class="login-card p-4 p-md-5">
          <div class="w-100">
            ...
            <div class="text-center login-stagger-1">
              <div class="login-icon-circle mx-auto"><i class="bi bi-key"></i></div>
              <h2 class="fw-bold mb-1" style="font-size:1.5rem; letter-spacing:-0.02em;">Redefinir Senha</h2>
              <p style="color: var(--login-muted); font-size:0.92rem;" class="mb-4">
                Digite a chave recebida por e-mail e defina sua nova senha.
              </p>
            </div>
            ...
            <form id="formRedefinir">
              ...
              <div class="form-group mb-3 login-stagger-2">
                <label for="inputChave" class="form-label">Chave de Verificação</label>
                <div class="input-group">
                  <span class="input-group-text"><i class="bi bi-shield-lock-fill"></i></span>
                  <input type="text" class="form-control code-input" id="inputChave" name="chave_hex"
                         placeholder="A1B2C3" required maxlength="6" pattern="[a-fA-F0-9]{6}"
                         title="6 caracteres hexadecimais" autocomplete="off">
                </div>
              </div>

              <div class="form-group mb-3 login-stagger-3">
                <label for="inputSenha" class="form-label">Nova Senha</label>
                <div class="input-group">
                  <span class="input-group-text"><i class="bi bi-lock-fill"></i></span>
                  <input type="password" class="form-control" id="inputSenha" name="nova_senha"
                         placeholder="Mínimo 6 caracteres" required minlength="6" autocomplete="new-password">
                  <button class="btn-toggle-pw" type="button" onclick="togglePw('inputSenha', this)" tabindex="-1">
                    <i class="bi bi-eye"></i>
                  </button>
                </div>
              </div>
              ...
              <div class="form-group mb-4 login-stagger-3">
                <label for="inputConfirmar" class="form-label">Confirmar Nova Senha</label>
                <div class="input-group">
                  <span class="input-group-text"><i class="bi bi-lock-fill"></i></span>
                  <input type="password" class="form-control" id="inputConfirmar" name="confirmar_senha"
                         placeholder="Repita a nova senha" required minlength="6" autocomplete="new-password">
                  <button class="btn-toggle-pw" type="button" onclick="togglePw('inputConfirmar', this)" tabindex="-1">
                    <i class="bi bi-eye"></i>
                  </button>
                </div>
              </div>

              <div class="d-grid mb-3 login-stagger-4">
                <button type="submit" class="btn btn-primary btn-lg fw-bold" id="btnRedefinir">
                  <i class="bi bi-check-circle me-2"></i>Redefinir Senha
                </button>
              </div>
            </form>

            <div class="text-center login-stagger-4">
  ...
  <script>
    ...
    document.getElementById('inputChave').addEventListener('input', function() {
      this.value = this.value.toUpperCase().replace(/[^0-9A-F]/g, '').substring(0, 6);
    });

    $('#formRedefinir').on('submit', function(e){
      e.preventDefault();
      var $btn = $('#btnRedefinir');
      var originalHtml = $btn.html();
      $('#alertErro, #alertSucesso').hide();

      if ($('#inputSenha').val() !== $('#inputConfirmar').val()) {
        $('#alertErroMsg').text('As senhas não coincidem.');
        $('#alertErro').css('display', 'flex');
        return;
      }

      $btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>Redefinindo...');
      ...
      $.ajax({
        ...
        success: function(res){
          if (res.sucesso) {
            $('#alertSucessoMsg').html(res.mensagem + ' <a href="' + baseUrl + '/login">Ir para o login</a>');
            $('#alertSucesso').css('display', 'flex');
            $('#formRedefinir').hide();
          } else {
            $('#alertErroMsg').text(res.erro || 'Erro ao redefinir senha');
            $('#alertErro').css('display', 'flex');
          }
