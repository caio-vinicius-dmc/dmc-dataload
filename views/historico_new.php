...
<div class="modal fade" id="modalDetalhes" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content modal-modern">
            ...
            <div class="modal-body-modern" id="modalDetalhesBody">
                <!-- Conteúdo dinâmico -->
            </div>
            <div class="modal-footer-modern">
                ...
                <button type="button" class="btn btn-modern-primary" id="btnReexecutar">
...
function renderizarDetalhes(dados) {
    ...
    console.log("🔍 dados.logs:", dados.logs);
    console.log("📊 Tipo de dados.logs:", typeof dados.logs, Array.isArray(dados.logs));
    
    const badgeClass = dados.status === "sucesso" ? "badge-success" : 
                      (dados.status === "falha" || dados.status === "erro" ? "badge-danger" : "badge-warning");
        ...
        <div class="row g-4">
            <div class="col-md-6">
                ...
                <table class="table table-sm">
                    <tr><th width="40%">ID Log:</th><td>${dados.id}</td></tr>
                    <tr><th>Rotina:</th><td><strong>${dados.nome_rotina || "-"}</strong></td></tr>
                    <tr><th>Status:</th><td><span class="badge-status ${badgeClass}">${dados.status}</span></td></tr>
                    <tr><th>Início:</th><td>${dados.data_inicio ? new Date(dados.data_inicio).toLocaleString("pt-BR") : "-"}</td></tr>
                    <tr><th>Fim:</th><td>${dados.data_fim ? new Date(dados.data_fim).toLocaleString("pt-BR") : "-"}</td></tr>
                    <tr><th>Duração:</th><td>${formatDuracao(dados.duracao_ms)}</td></tr>
                </table>
            </div>
            <div class="col-md-6">
                <h6 class="text-muted mb-3">Estatísticas</h6>
                <table class="table table-sm">
                    <tr><th width="50%">Blocos Executados:</th><td>${dados.blocos_executados || 0}</td></tr>
                    <tr><th>Blocos Sucesso:</th><td class="text-success">${dados.blocos_sucesso || 0}</td></tr>
                    <tr><th>Blocos Falha:</th><td class="text-danger">${dados.blocos_falha || 0}</td></tr>
                    <tr><th>Registros:</th><td>${dados.registros_processados?.toLocaleString("pt-BR") || "-"}</td></tr>
                    <tr><th>Arquivo CSV:</th><td>${dados.caminho_csv ? 
    ...
    if (dados.mensagem_erro) {
        html += `
            <div class="alert alert-danger mt-3">
                <h6><i class="bi bi-exclamation-triangle me-2"></i>Mensagem de Erro</h6>
                <pre class="mb-0 small" style="white-space: pre-wrap;">${dados.mensagem_erro}</pre>
            </div>
        `;
    }

    // CARDS DE BLOCOS EXECUTADOS
    if (dados.logs && Array.isArray(dados.logs) && dados.logs.length > 0) {
        ...
        dados.logs.forEach(log => {
            const tipo = (log.tipo || 'UNKNOWN').toUpperCase();
        ...
        // Botões de tipo
        Object.keys(stats.tipos).sort().forEach(tipo => {
            const icon = {
                'SELECT': 'search',
                'INSERT': 'plus-circle',
            ...
            html += `
                    <button class="btn btn-sm btn-outline-secondary filtro-bloco" data-filtro="tipo-${tipo.toLowerCase()}" onclick="filtrarBlocos('tipo-${tipo.toLowerCase()}')">
                        <i class="bi bi-${icon}"></i> ${tipo} <span class="badge bg-secondary ms-1">${stats.tipos[tipo]}</span>
                    </button>`;
        });
        
        html += `
                    <div class="vr"></div>
                    <button class="btn btn-sm btn-outline-info" onclick="ordenarBlocosPorTempo()">
                        <i class="bi bi-sort-numeric-down"></i> Ordenar por Tempo
                    </button>
                    <button class="btn btn-sm btn-outline-warning" onclick="limparFiltrosBlocos()">
        ...
        dados.logs.forEach((log, index) => {
                ...
                <div class="bloco-header ${isSuccess ? 'header-success' : 'header-error'}" onclick="toggleBlocoCollapse(${index})">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <strong>${log.bloco || `Bloco ${log.ordem || index + 1}`}</strong>
                            <span class="bloco-tipo-badge">${tipo}</span>
                        </div>
                        <div class="d-flex align-items-center gap-2">
                            <span class="bloco-status-badge ${isSuccess ? 'status-success' : 'status-error'}">${log.status?.toUpperCase() || 'DESCONHECIDO'}</span>
                            ${log.duracao_ms !== undefined ? `<span class="bloco-time-badge"><i class="bi bi-clock"></i> ${formatDuracao(log.duracao_ms)}</span>` : ''}
            ...
            // SQL
            if (log.sql) {
                const blocoId = log.bloco || index;
                html += `
                        <div class="mb-3 sql-container">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <label class="form-label fw-bold mb-0"><i class="bi bi-file-code"></i> SQL Executado:</label>
            ...
            if (log.erro) {
                html += `
                        <div class="erro-box mb-3">
                            ...
                            <div class="text-danger font-monospace small">${log.erro}</div>
                        </div>`;
            }
        ...
        console.warn('⚠️ Nenhum log de bloco encontrado ou dados.logs não é array');
        if (dados.logs) {
            console.log('📄 Tipo de dados.logs:', typeof dados.logs);
            console.log('📄 Conteúdo de dados.logs:', dados.logs);
        }
    }
